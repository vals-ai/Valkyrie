...
class MockClient:
    ...
    def get(
        ...
        *,
        ...
        self.url = url
        self.params = params
        if "/fetch-run-outputs/" in url or url.endswith("/outputs"):
...
def test_retrieve_results_adapts_canonical_wire_response_to_legacy_cli_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    run_id = uuid4()

    class CanonicalResultsClient(MockClient):
        def get(
            self,
            url: str,
            *,
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "status": "IN_PROGRESS",
                    "error_message": None,
                    "run_arguments": {"contract": {"name": "agent"}, "concurrency": 1},
                    "tasks_stopped": None,
    ...
    fake_client = CanonicalResultsClient()
    monkeypatch.setattr(TrackerService, "_load_config", staticmethod(_empty_config))
    monkeypatch.setattr(TrackerService, "parse_config_keys", _empty_config_keys)
    monkeypatch.setattr("valkyrie.cli.tracker_client.httpx.Client", lambda **_kwargs: fake_client)
...
def test_update_run_concurrency_uses_canonical_patch_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    """Send concurrency updates to the canonical run endpoint."""
    requests: list[httpx.Request] = []
    run_id = UUID("123e4567-e89b-12d3-a456-426614174000")
...
def test_retry_or_resume_sends_retry_mode(
    ...
    assert mock_client.url is not None and mock_client.url.endswith("/retry")
