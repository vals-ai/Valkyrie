...
### Fixed
- Keep showing capacity for scheduler pools that still have active (building/in-progress/evaluating) work after their waiting count drops to zero, instead of dropping the pool from the Dashboard (#802)
- Grant the tracker task role `lambda:InvokeFunction` on output lambdas, fixing an `AccessDeniedException` when `valk run results --preview` invokes the completion-callback lambda directly (#801)
