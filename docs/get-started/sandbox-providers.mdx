...
1. Create an API token under `Settings → Tokens → API tokens` in the Modal workspace that should own the sandboxes, for example [the `vals-ai` workspace](https://modal.com/settings/vals-ai/tokens/api-tokens). Modal shows the token once, as a CLI command:
