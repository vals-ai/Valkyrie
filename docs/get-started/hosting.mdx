...
| --- | --- |
| No `aws.credentials` object | Managed AWS. The tracker and executor host use deployment task roles. |
| `aws.credentials` with both access keys | Access-key AWS. The tracker receives the configured AWS resources and credentials. |
