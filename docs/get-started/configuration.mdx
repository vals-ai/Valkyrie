...
| Variable | Effect |
| --- | --- |
| `VALKYRIE_ENV` | `prod` (default) or `dev`. Selects the hosted tracker URL and the default configuration file. Any other value fails immediately. |
| `TRACKER_SERVICE_URL` | Overrides the tracker endpoint for both environments. |
...
With `VALKYRIE_ENV=dev` the CLI uses `https://benchmark-tracker-dev.vals.ai` and `~/.config/valkyrie/dev.yaml`, so a development profile never overwrites your production configuration.
