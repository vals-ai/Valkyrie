...
- `--update-agent` refreshes the frozen agent from the current bundle in the CLI's configured S3 bucket. It rejects a run stored in a different bucket before writing or resuming. Omit this option for owner runs to keep their frozen bundle.
...
Python callers can change an active run's limit with `await client.runs.update_concurrency(run_id, concurrency=10)`. Lowering the limit lets existing tasks finish and limits new admissions; see the [SDK method](/reference/sdk/runs#update-concurrency).
