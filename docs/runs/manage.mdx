...
- `--update-agent` refreshes the frozen agent from its current S3 bundle, so a re-pushed agent takes effect. Stop a run that is still in progress before using it.
