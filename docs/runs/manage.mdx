...
- `--update-agent` refreshes the frozen agent from its current S3 bundle, so a re-pushed agent takes effect. On a run still in progress, it applies only when tasks restart; otherwise stop the run first.
