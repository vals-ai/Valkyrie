...
valkyrie run errors <run-id> --format json
```

These commands show stored run and current task error messages.
...
| `--status` | Filter by `IN_PROGRESS`, `STOPPING`, `STOPPED`, `FINISHED`, or `ERROR` |
| `--order-by` | Sort `desc` or `asc` |
| `--started-by` | Comma-separated starter emails |

The interactive list supports `[h]` for previous, `[l]` for next, and `[q]` to quit. Emit all matching runs as one document with:
