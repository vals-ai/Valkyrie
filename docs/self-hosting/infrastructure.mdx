...
Synthesis reads the deployment-owned inventory from the environment, so export it before planning. A `prod` or `dev` stage fails with `ValueError: prod deployments require AWS_DEPLOYMENT_ROLE_ORG_IDS.` when these are unset:
...
`AWS_DEPLOYMENT_ROLE_ORG_IDS` lists the organization ids allowed to submit managed runs, and `AWS_TRACKER_SECRET_NAME_PREFIXES` the Secrets Manager name prefixes the tracker may resolve for benchmark-service authentication. Authenticated deployments also require `DESCOPE_PROJECT_ID` and `DESCOPE_MANAGEMENT_KEY_SECRET_NAME`.
