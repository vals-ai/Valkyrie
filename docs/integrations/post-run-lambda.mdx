---
title: Invoke a post-run Lambda
description: Run an AWS Lambda function after a benchmark finishes.
---

The `--lambda` flag on [`valkyrie run start`](/reference/cli/run#start) invokes an AWS Lambda function after a run completes.
