...
A benchmark can set separate outbound network policies for setup and evaluation:
...
Use `"*"` for unrestricted access, `[]` to block all outbound access, or a non-empty list to allow only those URLs, domains, IP addresses, or CIDRs. Both stages default to `"*"`, so existing benchmark services can omit `egress` without changing behavior. The selected setup policy remains active until the agent policy is applied. The evaluation policy remains active while Valkyrie evaluates the live task sandbox, including evaluation stream resumption.
