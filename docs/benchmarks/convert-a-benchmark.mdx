...
By default, Valkyrie runs the agent contract's install command before `setup_task`. Set `agent_install_order="after_setup"` on `RetrieveTaskResponse` when benchmark setup must prepare the runtime first. `ComposeSource` always runs setup before agent installation because setup starts the service that receives agent commands. This field controls the agent's install command only; `setup_task` remains responsible for benchmark and task dependencies.

A benchmark can set separate outbound network policies for setup, agent execution, and evaluation:

```python
egress=BenchmarkEgressPlan(
    setup_task=["https://artifacts.example.com"],
    run=[],
    evaluation=[],
)
```

Use `"*"` for unrestricted access, `[]` to block all outbound access, or a non-empty list to allow only those URLs, domains, IP addresses, or CIDRs. Setup and evaluation default to `"*"`. Run defaults to `None`, meaning the benchmark has no run-policy opinion; this preserves existing behavior. A legacy non-empty agent `egress_allowlist` adds its destinations to the benchmark run allowlist. The selected setup policy remains active until the agent installation policy is applied. The evaluation policy remains active while Valkyrie evaluates the live task sandbox, including evaluation stream resumption.

<Warning>
  Hosted sandboxes require `linux/amd64` images. Public images must pull anonymously. Private Amazon ECR images require the registry and its cross-account pull role to be configured in Daytona before Valkyrie creates the sandbox.
</Warning>
