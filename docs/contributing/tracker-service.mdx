...
`CloudRuntimeConfig.create_runtime()` composes the existing AWS storage, secret, and log adapters. API dependencies keep services alive for the request. Executors open a separate runtime and create sandbox providers on first use. Task cleanup waits for pending log writes before the executor closes its provider.

Run requests default to `environment: "aws"`; AWS is currently the only supported environment. Optional `properties` contains `region`, `s3_bucket`, `log_group`, and `log_retention_days`. New runs save the resolved resource settings with their arguments. Retry, resume, and artifact reads reuse those settings with freshly resolved AWS credentials. Managed submissions cannot override deployment resources. Caller-credential submissions may supply resource properties separately from the legacy harness configuration.

Older runs without properties retain legacy resource resolution. Do not infer their original bucket or region from today's defaults.
...
Add `include_capacity=true` to request best-effort CPU, memory, and disk capacity observations for each waiting pool. Capacity is returned only when every waiting run in that pool uses the same managed sandbox-provider configuration and the reconstructed provider still matches the persisted pool. Each `capacity_domains` entry keeps one canonical provider target and sandbox class separate; the API never combines distinct domains. Access-key, ambiguous, unsupported, timed-out, or unavailable provider configurations return `capacity_domains: null` without failing the queue snapshot, while an empty list means the provider successfully reported no domains. Values are observational and not reserved. Provider credentials and configuration identifiers remain internal.
...
Stop admission of starts, retries, and resumes. Stop and drain active runs before deploying Tracker and a compatible executor release. Activate that release, verify a new run and a stopped-run resume, then reopen admission.

The new fields use the existing arguments JSON column; no database column migration is required. Older executors reject the new argument fields. Do not roll back to those executors after new-format arguments have been saved without converting the affected records and preserving their resource settings.
