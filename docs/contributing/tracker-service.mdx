...
If an executor stops while sandbox creation may still be in flight, Tracker retains that pool's reservation. Inspect outstanding reservations with `uv run python -m tracker.executor.pool_reservation_recovery list` from `services/tracker`, using the target Tracker environment's `DATABASE_URL`. Before releasing one, confirm the original creator has stopped, the provider request has settled, and the sandbox is absent or cleaned up. Use the reservation ID from the listing and run `uv run python -m tracker.executor.pool_reservation_recovery release --pool-id <pool-id> --reservation-id <reservation-id> --confirm-creator-stopped --confirm-provider-settled --provider-outcome absent` when no sandbox was created, or use `--provider-outcome cleaned` after removing it. Dispatch terminal status or lease expiry alone does not make a reservation safe to release. Provider sandbox names include random suffixes; use the run and task labels when checking provider state.
...
To check an older protocol-4 package against this Tracker version, set `TEST_EXECUTOR_COMPAT_PEX` to its absolute path and run `uv run pytest tests/integration/local/database/test_dispatch_api.py::test_api_server_restart_preserves_executor_process`. This uses the unchanged package's client and persistence code against a restarted HTTP server on a database created by Alembic migrations. It checks the original process and attempt finish once; it does not launch an ECS host or exercise the package entrypoint.
