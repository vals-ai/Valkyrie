...
Set separate outbound network policies for agent installation and execution:
...
Each stage accepts one of these values:

- `"*"` keeps outbound access unrestricted.
- `[]` blocks all outbound access.
- A non-empty list allows only the listed URLs, domains, IP addresses, or CIDRs.

Both `install` and `run` default to `"*"`. You can restrict one stage without changing the other:
...
For standard sandboxes, Valkyrie uploads the agent bundle, installs the agent, sets up the benchmark task, runs the agent, then evaluates the result. Compose tasks are the compatibility exception: benchmark setup first starts the Compose service, then Valkyrie installs and runs the agent inside that service. The agent contract owns the `install` and `run` policies. The benchmark owns its setup and evaluation policies. Each policy remains active until Valkyrie applies the next stage's policy.

Existing contracts can keep using `egress_allowlist` for `run_cmd`. Omit `egress`, and a missing or empty legacy allowlist keeps installation and execution unrestricted. A non-empty legacy allowlist restricts only agent execution. Do not combine a non-empty `egress_allowlist` with `egress`.

Allowlists help keep evaluations clean, but shared CDN edge IPs can make network restrictions broader than a single hostname.
