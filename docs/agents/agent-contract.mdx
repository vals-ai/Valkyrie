...
When `run_cmd` exits non-zero, the task error records only the exit code. To surface the original error, write a short `Type: message` (first 2048 characters are kept) to the file at `$VALKYRIE_ERROR_PATH` before exiting; agent stdout/stderr is never persisted in the error.
