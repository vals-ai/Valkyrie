...
Optional command to run in the agent's sandbox and working directory after a known `run_cmd` exit and before output collection. It uses the same placeholders and contract kwargs as `run_cmd`, and has a five-minute timeout. The executable and its dependencies must already be available from the agent bundle or `install_cmd`.

Finalization is skipped when execution ownership was revoked, the tracker was cancelled, or the sandbox/provider was lost before the agent exit was known. Valkyrie still attempts output collection when finalization fails. The agent's failure takes precedence over a finalization failure; otherwise, the finalization failure is reported. This hook only follows the tracked shell's exit; it does not guarantee that descendant processes have stopped.

```yaml
finalize_cmd: "python -m agent.finalize --task {task_id}"
```

Omit this field to disable finalization.
...
For private trajectory custody, keep the producer's native trajectory in `final_output` and write an optional ATIF conversion to a separate `output_artifacts` source. `finalize_cmd` runs in the same sandbox after the agent exits, then Valkyrie attempts the native archive before the ATIF sidecar. Valkyrie does not parse the native format or replace its bytes. Valkyrie attempts the remaining collection after a finalization or archive failure, then reports the earlier failure. Collection cannot continue after the sandbox or executor host is lost.
...
Define typed parameters with defaults that get substituted into `run_cmd` and `finalize_cmd`:
