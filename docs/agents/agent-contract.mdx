...
Finalization is skipped when execution ownership was revoked, the tracker was cancelled, or the sandbox/provider was lost before the agent exit was known. A finalization failure does not replace an existing agent failure, and Valkyrie still attempts output collection. This hook only follows the tracked shell's exit; it does not guarantee that descendant processes have stopped.
