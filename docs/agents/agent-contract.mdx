...
### `install_egress: "*" | list[str]`

Set the outbound network policy used while Valkyrie installs the agent:
...
`install_egress` accepts one of these values:
...
Installation defaults to `"*"`. Nested `egress` configuration is not supported. The benchmark owns setup, agent-run, and evaluation policies. The benchmark also chooses whether agent installation occurs before or after setup. Compose tasks always run setup first so setup can start the service that receives agent commands. Each policy remains active until Valkyrie applies the next stage's policy.

Existing contracts can keep using `egress_allowlist` as a legacy agent-run request. A missing or empty legacy allowlist adds no destinations. A non-empty legacy allowlist is unioned with the benchmark's run allowlist. When the benchmark has no run-policy opinion, a missing or empty legacy allowlist preserves unrestricted execution and a non-empty list preserves the old allowlist behavior. `"*"` from the benchmark keeps execution unrestricted.
