...
def _merge_statistics(metadata: dict) -> dict:
    ...
    for turn in metadata["turns"]:
        ...
        metadata["total_tokens"]["total_tokens"] += turn["in_tokens"] + turn["out_tokens"]

        if "tokens_retrieval" in turn:
            tr = turn["tokens_retrieval"]
            metadata["total_tokens_retrieval"]["prompt_tokens"] += tr.get("prompt_tokens", 0)
            metadata["total_tokens_retrieval"]["completion_tokens"] += tr.get("completion_tokens", 0)
            metadata["total_tokens_retrieval"]["total_tokens"] += tr.get("total_tokens", 0)
