...
class ValkyrieConfig(BaseModel):
    ...
    aws_secret_access_key: str = Field(alias="AWS_SECRET_ACCESS_KEY", repr=False)
    aws_default_region: str = Field(alias="AWS_DEFAULT_REGION")
    aws_session_token: str | None = Field(default=None, alias="AWS_SESSION_TOKEN", repr=False)
    ...
    benchmark_auth: dict[str, str] = Field(default_factory=dict, repr=False)
    ...
    @classmethod
    def from_yaml(cls, path: str | Path = DEFAULT_CONFIG_PATH) -> Self:
