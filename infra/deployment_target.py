...
def target_from_environment(environment: Mapping[str, str]) -> DeploymentTarget:
    ...
    expected_account_id = _required(environment, "DEV_ACCOUNT_ID") if stage == DEV else PRODUCTION_ACCOUNT_ID
    if not _ACCOUNT_ID_PATTERN.fullmatch(expected_account_id):
        account_variable = "DEV_ACCOUNT_ID" if stage == DEV else "production account ID"
        raise DeploymentTargetError(f"{account_variable} must be a 12-digit AWS account ID.")
    if stage == DEV and expected_account_id == PRODUCTION_ACCOUNT_ID:
    ...
    if cdk_account_id != expected_account_id:
        raise DeploymentTargetError(
            f"CDK_DEFAULT_ACCOUNT must be {expected_account_id} for {stage}; got {cdk_account_id}."
        )
    ...
    return DeploymentTarget(stage=stage, account_id=expected_account_id, region=region)
...
def main() -> int:
    try:
        ...
        boto_sts_client: BotoStsClient = session.client(  # pyright: ignore[reportUnknownMemberType]
            "sts", region_name=target.region
        )
        sts_client = cast(StsClient, boto_sts_client)
        validate_sts_caller(target, sts_client)
