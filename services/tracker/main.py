...
def _resolve_contract_from_s3(request: StartBenchmarkRequest) -> AgentContractRequest:
...
async def start_benchmark(
    ...
    if not request.contract.install_cmd and not request.contract.run_cmd:
        request = request.model_copy(update={"contract": _resolve_contract_from_s3(request)})
...
async def retrieve_results(
    ...
    if s3:
        ...
        presigned_url = create_presigned_url(s3_key, harness_config.aws, harness_config.s3_bucket, expiration=86400)
...
async def fetch_agent_outputs(
    ...
    if s3:
        ...
        presigned_url = create_presigned_url(s3_tar_key, harness_config.aws, harness_config.s3_bucket, expiration=300)
