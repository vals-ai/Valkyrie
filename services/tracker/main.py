...
async def retrieve_results(
    ...
    lambda_function: str | None = Query(
        default=None,
        min_length=1,
        max_length=64,
    ...
    With `s3=True`, ordinary retrieval uploads the selected view to the canonical result key.
    When `lambda_function` is supplied, the selected view instead uses a request-specific key
    so the callback cannot overwrite the canonical result object.
    ...
    if lambda_function and AUTH_REQUIRED:
        access_key_id = resolve_descope_identity(extract_api_key(http_request)).access_key_id
        if benchmark_row.started_by_id is None or benchmark_row.started_by_id != access_key_id:
            raise HTTPException(
                status_code=403,
                detail="Only the credential that started this run can invoke its callback",
            )
    ...
    if s3:
        callback_s3_key = (
            f"{S3_BENCHMARKS_PREFIX}/{benchmark_id}/result-callbacks/results/{uuid4()}.json"
            if lambda_function
            else None
        )
        s3_key = await upload_final_view(benchmark_row, final_view, aws_runtime, s3_key=callback_s3_key)
