...
async def start_benchmark(
    ...
    # Create and admit the run only after pre-flight checks pass.
    ...
    try:
        ...
        for task_id in verify_response.task_ids:
            session.add(Task(org_id=run_row.org_id, benchmark=run_row.id, task_id=task_id))
        ...
        raise TrackerServiceError("Failed to admit run execution") from exc
...
async def stop_benchmark(
    ...
    run_row = fetch_run_row(benchmark_id, session, org, for_update=True)
    if run_row.status not in valid_stop_states:
        raise HTTPException(
            status_code=400,
            detail=f"Run {benchmark_id} is currently in the {run_row.status} state. Can only pause an in progress or error run.",
...
async def retry_or_resume_benchmark(
    ...
    try:
        ...
        pre_action_status = run_row.status
        ...
        resume_request = run_row.start_benchmark_request(
            harness_config,
            service_headers=effective_service_headers,
        )
...
async def fetch_run_outputs(
    ...
    keys = _output_keys(run_prefix, task_ids, harness_config, benchmark_id)
    first_key = await anext(keys, None)
    if first_key is None:
        raise HTTPException(status_code=404, detail=f"No outputs found for run '{benchmark_id}'")

    return StreamingResponse(
        _tar_output_stream(_output_keys_with_first(first_key, keys), run_prefix, harness_config),
