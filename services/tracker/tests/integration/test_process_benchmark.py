...
def _create_benchmark(
    ...
    benchmark = start_benchmark_request_to_benchmark(request, Org(id=VALS_ORG_ID, name="Vals"))
...
async def test_process_task(
    ...
    task_row = Task(org_id=VALS_ORG_ID, task_id=_TASK_ID, benchmark=benchmark.id)
    ...
    await process_task(task_row, request, benchmark_service, benchmark.id, _TASK_ID, harness_config, Org(id=VALS_ORG_ID, name="Vals"))
