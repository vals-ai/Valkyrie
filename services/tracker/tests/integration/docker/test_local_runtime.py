...
from benchmark_service import DockerProviderConfig, ImageSource, Resources, SandboxQuery
...
async def test_local_runtime_transfers_and_executes_frozen_agent(tmp_path: Path) -> None:
    ...
    config = LocalRuntimeConfig(
        ...
        docker=DockerProviderConfig.from_env().model_copy(update={"installation_id": f"tracker-test-{uuid4().hex}"}),
    )
    contract = AgentContractRequest(name="local-test", run_cmd="/bundle/local-test/run.sh")
    stream = io.BytesIO()
    with zipfile.ZipFile(stream, "w") as archive:
        archive.writestr("local-test/contract.yaml", "name: local-test\ninstall_cmd: ''\nrun_cmd: /bundle/local-test/run.sh {problem_statement_path}\n")
    ...
    async with LocalRuntimeFactory.open(config, org_id) as runtime:
        ...
        async with runtime.get_sandbox_provider(provider_config) as provider:
            ...
            assert [sandbox async for sandbox in provider.list_sandboxes(SandboxQuery(labels={}))] == []
