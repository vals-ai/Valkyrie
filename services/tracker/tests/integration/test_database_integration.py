...
class TestDatabaseIntegration:
    async def _create_task(self, database_session: Session, task_id: str, benchmark_id: UUID) -> Task:
        task_row = Task(org_id=VALS_ORG_ID, task_id=task_id, benchmark=benchmark_id)
    ...
    async def _create_evaluation_result(
        ...
        evaluation_result_row = EvaluationResult(
            org_id=VALS_ORG_ID, task=task_row.id, instance_id=str(uuid.uuid4()), result=evaluation_result
        )
    ...
    async def test_database_integrity(self, database_session: Session, example_benchmark_object: Benchmark):
        ...
        # Test the task table
        task_row = Task(org_id=VALS_ORG_ID, task_id=random_task_id(), benchmark=benchmark_row.id)
        database_session.add(task_row)
