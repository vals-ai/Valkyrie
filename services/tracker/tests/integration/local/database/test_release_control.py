...
from tracker.utils.run_control import apply_stop_run
...
def test_whole_stop_and_retry_serialize_on_the_benchmark_row(
    ...
    def whole_stop(session: Session, benchmark_id: UUID) -> None:
        benchmark = fetch_run_row(benchmark_id, session, org, for_update=True)
        ...
        apply_stop_run(benchmark, session, force=True, org=org)
        session.flush()
