...
def _seed(session: Session, n: int, user_id=None, start_status=None) -> list[Benchmark]:
...
def test_filter_by_run_by_user_id(client, database_session):
    alice = User(org_id=TEST_ORG_ID, descope_user_id="U_alice", email="alice@x.com")
    database_session.add(alice)
    database_session.commit()
    _seed(database_session, 2, user_id=alice.id)
    _seed(database_session, 2, user_id=None)  # unattributed

    resp = client.get(
        f"/fetch-benchmarks?run_by_user_id={alice.id}",
        headers={"x-api-key": "fake-key"},
    )
    data = resp.json()
...
def test_table_row_includes_run_by_email(client, database_session):
    user = User(org_id=TEST_ORG_ID, descope_user_id="U_email", email="emailtest@x.com")
    database_session.add(user)
    database_session.commit()
    _seed(database_session, 1, user_id=user.id)
...
def test_table_row_run_by_email_null_when_unattributed(client, database_session):
    _seed(database_session, 1, user_id=None)
    resp = client.get("/fetch-benchmarks", headers={"x-api-key": "fake-key"})
    data = resp.json()
