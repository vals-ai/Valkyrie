...
def test_list_agents_uses_harness_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        agents_api,
        "list_s3_agent_names",
        lambda **_kwargs: [{"name": "agent-a", "last_modified": datetime(2026, 1, 2, tzinfo=timezone.utc)}],
    )
...
def test_agent_download_url_uses_module_ttl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_presigned_url(*_args, expiration: int, **_kwargs) -> str:
        captured_expiration.append(expiration)
        return "https://example.test/agent-a.zip"

    monkeypatch.setattr(agents_api, "s3_object_exists", lambda *_args, **_kwargs: True)
...
def test_agent_download_url_returns_404_for_missing_agent(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(agents_api, "s3_object_exists", lambda *_args, **_kwargs: False)
