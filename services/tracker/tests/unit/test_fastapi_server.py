...
class TestFastapiServer:
    ...
    async def test_fetch_benchmark(self, database_session: Session, example_benchmark_object: Benchmark):
        ...
        task_rows = [Task(org_id=VALS_ORG_ID, task_id=f"task_{i}", benchmark=benchmark_row.id) for i in range(10)]
    ...
    async def test_retrieve_results(self, database_session: Session, example_benchmark_object: Benchmark):
        ...
        task_rows = [
            Task(org_id=VALS_ORG_ID, task_id=f"task_{i}", benchmark=benchmark_row.id, status=TaskStatus.FINISHED) for i in range(10)
        ]
        evaluation_result_rows = [
            EvaluationResult(org_id=VALS_ORG_ID, task=task_row.id, instance_id=str(uuid4()), result={"finished": True})
        ...
        final_evaluation_row = FinalEvaluation(
            org_id=VALS_ORG_ID, benchmark=benchmark_row.id, final_score=100, properties={"resolved_tasks": [], "unresolved_tasks": []}
        )
        ...
        task_rows = [
            Task(org_id=VALS_ORG_ID, task_id=f"task_{i}", benchmark=benchmark_row.id, status=TaskStatus.STOPPED) for i in range(11, 21)
        ]
        database_session.add_all(task_rows)
        database_session.commit()
    ...
    async def test_fetch_benchmarks(self, database_session: Session, example_benchmark_object: Benchmark):
        ...
        benchmark_rows = [
            Benchmark(org_id=VALS_ORG_ID, id=uuid4(), name="swebench", arguments=example_benchmark_object.arguments) for _ in range(4)
        ]
