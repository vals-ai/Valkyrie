...
_SANDBOX_SECRET_KEYS: dict[str, dict[str, str]] = {
    "daytona": {
        "DAYTONA_API_KEY": "x-api-key",
        "DAYTONA_API_URL": "x-api-url",
        "DAYTONA_TARGET": "x-target",
    },
    "modal": {
        "MODAL_TOKEN_ID": "x-modal-token-id",
        "MODAL_TOKEN_SECRET": "x-modal-token-secret",
    },
}

_SANDBOX_DELETE_BATCH_SIZE = 10


def fetch_sandbox_headers(sandbox_provider: str, daytona_secret_name: str, aws: AWSCredentials) -> dict[str, str]:
    """Fetch sandbox provider credentials from AWS Secrets Manager and return as headers."""
    key_mapping = _SANDBOX_SECRET_KEYS.get(sandbox_provider)
    if key_mapping is None:
        raise TrackerServiceError(f"Unsupported sandbox provider: {sandbox_provider}")

    secret = fetch_aws_secret(daytona_secret_name, aws)

    if not isinstance(secret, dict):
        raise TrackerServiceError(f"Expected a dict with all sandbox keys inside, received a string {secret}")

    missing_keys = set(key_mapping.keys()) - set(secret.keys())
    if missing_keys:
        raise TrackerServiceError(f"Missing following keys for {sandbox_provider}: {', '.join(missing_keys)}")

    missing_values = [key for key, value in secret.items() if key in key_mapping and not value]
    if missing_values:
        raise TrackerServiceError(f"Missing values for the following keys: {', '.join(missing_values)}")

    headers: dict[str, str] = {"x-sandbox-provider": sandbox_provider}
    for secret_key, header_key in key_mapping.items():
        headers[header_key] = secret[secret_key]
...
def create_benchmark_service_client(
    ...
    """Create a BenchmarkServiceClient using sandbox credentials from AWS Secrets Manager."""
    headers = fetch_sandbox_headers(sandbox_provider, daytona_secret_name, aws)
    if service_headers:
...
async def force_stop_sandboxes(
    ...
    """
    async with benchmark_row.benchmark_service(daytona_secret_name, aws) as benchmark_service:
        provider = await benchmark_service.get_sandbox_provider()
        ...
        query = SandboxQuery(
            ...
            limit=_SANDBOX_DELETE_BATCH_SIZE,
        )
        ...
        while True:
            ...
            async for sandbox in provider.list_sandboxes(query):
                if sandbox.id in failed_sandbox_ids:
                    continue
                batch.append(sandbox)
            ...
            for sandbox in batch:
                ...
                if result is None:
                    ...
                    failed_sandbox_ids.add(sandbox.id)
