...
def _load_task_or_404(
    benchmark_id: UUID, task_id: str, org: Org, session: Session
...
def _load_aws_or_none(
    org: Org, session: Session
) -> tuple[AWSCredentials | None, str | None]:
...
def get_single_task(
    ...
    return SingleTaskResponse(
        ...
        agent_caused_exit_reason=(
            eval_row.agent_caused_exit_reason.value
            if eval_row and eval_row.agent_caused_exit_reason
            else None
        ),
    )
    ...
    return FilesResponse(
        files=[
            FileEntry(
                ...
                last_modified=(
                    str(r["last_modified"]) if r.get("last_modified") else None
                ),
            )
            for r in rows
        ]
