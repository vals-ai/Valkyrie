...
_MAX_DIFF_LINES = 80
_REFERENCE_ROOT = Path("reference")
_CLI_REFERENCE_ROOT = _REFERENCE_ROOT / "cli"
_SDK_REFERENCE_ROOT = _REFERENCE_ROOT / "sdk"
_SDK_TYPES_REFERENCE_ROOT = _SDK_REFERENCE_ROOT / "models"
_CLI_INDEX_PATH = _CLI_REFERENCE_ROOT / "index.mdx"
_CLI_NAVIGATION_PATH = _CLI_REFERENCE_ROOT / "navigation.json"
_REDIRECTS_PATH = _REFERENCE_ROOT / "redirects.json"
_SDK_INDEX_PATH = _SDK_REFERENCE_ROOT / "index.mdx"
_SDK_CLIENT_PATH = _SDK_REFERENCE_ROOT / "client.mdx"
_SDK_TYPES_INDEX_PATH = _SDK_TYPES_REFERENCE_ROOT / "index.mdx"
_SDK_ERRORS_PATH = _SDK_REFERENCE_ROOT / "errors.mdx"
_SDK_NAVIGATION_PATH = _SDK_REFERENCE_ROOT / "navigation.json"
...
def _sdk_type_family_page_path(family: str) -> Path:
    ...
    return (_SDK_TYPES_REFERENCE_ROOT / family.lower()).with_suffix(".mdx")
...
def _legacy_type_route(entry: SDKModelReference | SDKEnumReference) -> str:
    ...
    return "/" + (_SDK_TYPES_REFERENCE_ROOT / entry.family.lower() / entry.slug).as_posix()
...
def _validate_type_families(entries: Sequence[SDKModelReference | SDKEnumReference]) -> None:
    """Reject public SDK type families without a generated family page."""
    actual = {entry.family for entry in entries}
    families = tuple(_SDK_TYPE_FAMILY_CARDS)
    expected = set(families)
    if actual != expected:
        raise ValueError(f"SDK type families must match {families}; got {tuple(sorted(actual))}")
...
def _cli_group_page_path(group: str) -> Path:
    ...
    return (_CLI_REFERENCE_ROOT / group).with_suffix(".mdx")
...
def _cli_legacy_command_route(command: CLICommandReference) -> str:
    ...
    return "/" + _CLI_REFERENCE_ROOT.joinpath(*command.path).as_posix()
...
def _sdk_resource_page_name(resource: SDKResourceReference) -> str:
    """Derive an SDK resource page name from its public client attribute."""
    return resource.client_attribute.rsplit(".", 1)[-1]
...
def _sdk_resource_page_path(resource_page: str) -> Path:
    ...
    return (_SDK_REFERENCE_ROOT / resource_page).with_suffix(".mdx")
...
def _sdk_legacy_method_route(resource_page: str, method: SDKMethodReference) -> str:
    ...
    return "/" + (_SDK_REFERENCE_ROOT / resource_page / method.name).as_posix()
...
def sdk_redirects(reference: SDKReference | None = None) -> list[dict[str, str]]:
    ...
    return [
        {
            "source": _sdk_legacy_method_route(_sdk_resource_page_name(resource), method),
            "destination": _sdk_method_route(_sdk_resource_page_name(resource), method),
        }
...
def _render_sdk_index(reference: SDKReference) -> str:
    ...
    lines.extend(
        (
            ...
            "## Client",
            "",
            f"Use [`ValkyrieClient`](/{_route(_SDK_CLIENT_PATH)}) to configure the async client and access resource namespaces.",
            "",
            "## Resource namespaces",
            "",
            "<CardGroup cols={2}>",
    ...
    for resource in reference.resources:
        page = _sdk_resource_page_name(resource)
    ...
    lines.extend(
        (
            ...
            "## Types and errors",
            "",
            f"Browse [public models and enums](/{_route(_SDK_TYPES_INDEX_PATH.parent)}) or the [SDK error hierarchy](/{_route(_SDK_ERRORS_PATH)}).",
            "",
        )
    )
    return "\n".join(lines)
...
def _sdk_navigation(reference: SDKReference) -> list[object]:
    ...
    navigation.extend(
        _route(_sdk_resource_page_path(_sdk_resource_page_name(resource))) for resource in reference.resources
    )
    type_pages: list[object] = [_route(_SDK_TYPES_INDEX_PATH)]
    type_pages.extend(_route(_sdk_type_family_page_path(family)) for family in _SDK_TYPE_FAMILY_CARDS)
...
def _render_types_index(reference: SDKReference) -> str:
    ...
    for family in _SDK_TYPE_FAMILY_CARDS:
